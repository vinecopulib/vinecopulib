...
void Hfunc(int* family, int* n, double* u, double* v, double* theta, double* nu, double* out)
{
    ...
    for(j=0;j<*n;j++)
    {
        ...
        else
        {
            ...
            {
                x = (gsl_cdf_ugaussian_Pinv(u[j]) - *theta*gsl_cdf_ugaussian_Pinv(v[j]))/sqrt(1.0-pow(*theta,2.0));
                if (isfinite(x))
                    h[j] = gsl_cdf_ugaussian_P(x);
                else if ((gsl_cdf_ugaussian_Pinv(u[j]) - *theta*gsl_cdf_ugaussian_Pinv(v[j])) < 0)
                    h[j] = 0;
                else
                    h[j] = 1;
            }
            else if(*family==2) //student
            {
                ...
                t1 = gsl_cdf_tdist_Pinv(u[j],*nu); t2 = gsl_cdf_tdist_Pinv(v[j],*nu); mu = *theta*t2; sigma2 = ((*nu+t2*t2)*(1.0-*theta*(*theta)))/(*nu+1.0);
            ...
            {
                double t1,t2,t3;
                t1=gsl_cdf_gamma_Pinv(1.0-u[j],*theta,1);
                t2=gsl_cdf_gamma_Pinv(1.0-v[j],*theta,1);
...
{
    double *negv, *negu;
    negv = (double*) calloc(*n,sizeof(double));
    negu = (double*) calloc(*n,sizeof(double));
    double ntheta, nnu;
    int nfamily;
    ntheta = -*theta;
    nnu = -*nu;
...
void Hinv(int* family, int* n, double* u, double* v, double* theta, double* nu, double* out)
{
    ...
    for(j=0;j<*n;j++)
    {
        ...
        {
            hinv[j] = gsl_cdf_ugaussian_P(gsl_cdf_ugaussian_Pinv(u[j])*sqrt(1.0-pow(*theta,2.0))+*theta*gsl_cdf_ugaussian_Pinv(v[j]));
        }
        else if(*family==2) //student
        {
            double temp1, temp2, mu, var;
            temp1 = gsl_cdf_tdist_Pinv(u[j],*nu+1.0); temp2 = gsl_cdf_tdist_Pinv(v[j],*nu); mu = *theta*temp2; var=((*nu+(temp2*temp2))*(1.0-(*theta*(*theta))))/(*nu+1.0);
            hinv[j] = gsl_cdf_tdist_P((sqrt(var)*temp1)+mu,*nu);
        }
        else if(*family==3) //clayton
        {
            if(*theta < XEPS) {
        ...
        else if(*family==41)	// 1-parametric asymmetric copula (Harry Joe)
        {
            double de=*theta;
            double tem1,y;
            tem1=gsl_cdf_gamma_Pinv(1.0-v[j],de,1);
            y=pow(tem1-log(u[j]),de) - pow(tem1,de);
            y=pow(y,(1.0/de));
            hinv[j]=1.0-gsl_cdf_gamma_P(y,de,1);
        }
