...
namespace vinecopulib
{
    ...
    std::vector<Eigen::MatrixXd> ClaytonBicop::tau_to_parameters(const double& tau)
    {
        std::vector<Eigen::MatrixXd> parameters = {Eigen::VectorXd(1)};
        parameters[0] << 2 * std::fabs(tau) / (1 - std::fabs(tau));
        return parameters;
    }

    double ClaytonBicop::parameters_to_tau(const std::vector<Eigen::MatrixXd>& parameters)
    {
        double tau = parameters[0](0) / (2 + std::fabs(parameters[0](0)));
        return flip_tau(tau);
    }

    std::vector<Eigen::MatrixXd> ClaytonBicop::get_start_parameters(const double tau)
    {
