...
TEST_P(TrafokernelTest, trafo_kernel_timer) {
    Timer timer;

    timer.start();
    bicop_.fit(u, controls);
    std::cout << "fit: " << timer.elapsedMilliseconds() << std::endl;
    timer.start();

    EXPECT_GE(bicop_.pdf(u).minCoeff(), 0.0);
    std::cout << "pdf: " << timer.elapsedMilliseconds() << std::endl;
    timer.start();

    EXPECT_GE(bicop_.hfunc1(u).minCoeff(), 0.0);
    std::cout << "hfunc1: " << timer.elapsedMilliseconds() << std::endl;
    timer.start();

    EXPECT_GE(bicop_.hfunc2(u).minCoeff(), 0.0);
    std::cout << "hfunc2: " << timer.elapsedMilliseconds() << std::endl;
    timer.start();

    EXPECT_GE(bicop_.hinv1(u).minCoeff(), 0.0);
    std::cout << "hinv1: " << timer.elapsedMilliseconds() << std::endl;
    timer.start();

    EXPECT_GE(bicop_.hinv2(u).minCoeff(), 0.0);
    std::cout << "hinv2: " << timer.elapsedMilliseconds() << std::endl;
}
