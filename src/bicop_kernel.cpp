...
namespace vinecopulib
{
    KernelBicop::KernelBicop()
    {
        ...
        interp_grid_ = InterpolationGrid(grid_points, MatrixXd::Constant(30, 30, 1.0));
    }

    VectorXd KernelBicop::pdf_default(const MatrixXd& u)
    {
        return interp_grid_.interpolate(u);
    }
    VectorXd KernelBicop::hfunc1_default(const MatrixXd& u)
    {
        return interp_grid_.intergrate_1d(u, 1);
    }
    VectorXd KernelBicop::hfunc2_default(const MatrixXd& u)
    {
        return interp_grid_.intergrate_1d(u, 2);
    }
    VectorXd KernelBicop::hinv1_default(const MatrixXd& u)
    {
        return hinv1_num(u);
    }
    VectorXd KernelBicop::hinv2_default(const MatrixXd& u)
    {
        return hinv2_num(u);
    }

    // TODO
    double KernelBicop::parameters_to_tau(const VectorXd &)
    {
