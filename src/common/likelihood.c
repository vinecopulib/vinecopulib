...
void archCDF(double* u, double* v, int* n, double* param, int* copula, double* out)
{
    ...
    for(j=0;j<*n;j++)
    {
        ...
        else
        {
            ...
            {
                t1=gsl_cdf_gamma_Pinv(1.0-u[j],param[0],1);
                t2=gsl_cdf_gamma_Pinv(1.0-v[j],param[0],1);
                t3=pow(pow(t1,param[0])+pow(t2,param[0]),(1.0/param[0]));
                out[j]=1.0-gsl_cdf_gamma_P(t3,param[0],1);
            }
        }
    }
...
void LL(int* family, int* n, double* u, double* v, double* theta, double* nu, double* loglik)
{
    ...
    else if(*family==1) //Gaussian
    {
        ...
        {
            dat[0]=u[j]; dat[1]=v[j];
            t1 = gsl_cdf_ugaussian_Pinv(dat[0]); t2 = gsl_cdf_ugaussian_Pinv(dat[1]);
    ...
    else if(*family==2) //Student
    {
        ...
        {
            dat[0] = u[j]; dat[1] = v[j];
            t1 = gsl_cdf_tdist_Pinv(dat[0],*nu); t2 = gsl_cdf_tdist_Pinv(dat[1],*nu);
            f = StableGammaDivision((*nu+2.0)/2.0,*nu/2.0)/(*nu*pi*sqrt(1.0-pow(rho,2.0))*gsl_ran_tdist_pdf(t1,*nu)*gsl_ran_tdist_pdf(t2,*nu))*pow(1.0+(pow(t1,2.0)+pow(t2,2.0)-2.0*rho*t1*t2)/(*nu*(1.0-pow(rho,2.0))),-(*nu+2.0)/2.0);
            if(log(f)>XINFMAX) ll += log(XINFMAX);
            else if(f < DBL_MIN) ll += log(DBL_MIN);
            else ll += log(f);
        }
    ...
    else if(*family==41)		// New: 1-parametric asymmetric copula (from Harry Joe)
    {
        ...
        {
            dat[0] = 1-u[j]; dat[1] = 1-v[j];
            tem1=gsl_cdf_gamma_Pinv(1.0-dat[0],*theta,1);
            tem2=gsl_cdf_gamma_Pinv(1.0-dat[1],*theta,1);
            con=gsl_sf_gamma(1.0+(*theta))/(*theta);
    ...
    else if(*family==51)		// New: rotated 1-parametric asymmetric copula (from Harry Joe)
    {
        ...
        for(j=0;j<*n;j++)
        {
            tem1=gsl_cdf_gamma_Pinv(1.0-u[j],*theta,1);
            tem2=gsl_cdf_gamma_Pinv(1.0-v[j],*theta,1);
            con=gsl_sf_gamma(1.0+(*theta))/(*theta);
            sm=pow(tem1,*theta)+pow(tem2,*theta);
            tem=pow(sm,(1.0/(*theta)));
            f=con*tem*exp(-tem+tem1+tem2)/sm;
