...
namespace vinecopulib
{
    StudentBicop::StudentBicop()
    {
        ...
        parameters_[0] << 0, 50;
        parameters_lower_bounds_[0] << -1, 2;
        parameters_upper_bounds_[0] << 1, 50;
    }

    Eigen::VectorXd StudentBicop::pdf_default(const Eigen::MatrixXd& u)
    {
        double rho = double(this->parameters_[0](0));
        double nu = double(this->parameters_[0](1));
    ...
    std::vector<Eigen::MatrixXd> StudentBicop::get_start_parameters(const double tau)
    {
        std::vector<Eigen::MatrixXd> parameters = tau_to_parameters(tau);
        parameters[0](1) = 8;
        return parameters;
    }
}
