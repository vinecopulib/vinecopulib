...
namespace test_bicop_sanity_checks {
    ...
    TEST(bicop_sanity_checks, catches_wrong_parameter_size) {
        for (auto family : bicop_families::nonparametric) {
            EXPECT_ANY_THROW(Bicop::create(family, 0, {Eigen::VectorXd::Zero(1)}));
        }
        for (auto family : bicop_families::one_par) {
            EXPECT_ANY_THROW(Bicop::create(family, 0, {Eigen::VectorXd()}));
            EXPECT_ANY_THROW(Bicop::create(family, 0, {Eigen::VectorXd::Zero(2)}));
        }
    ...
    TEST(bicop_sanity_checks, catches_parameters_out_of_bounds) {
            ...
            Eigen::VectorXd wrong_par = Eigen::VectorXd::Constant(1, 1.01);
